    ...
    public int evaluate(double avg10to89 ,double slope9to49,double slope50to90,int ClusterMutationTopCount,int max_no, double slope, double smoothness, double standarddeviation, String peakindicatorvector, String peakindicatorvector_2, double top40indexaggregation, double top15andtail15, String flag, double xgxs, double Top40AndLast60Broadcast, double AvgTail15_85,double ave_if_nl,String BAND,double NUM_VALID_RBs,double avg0to9,double avg45to49) {
        try {
            ...
            else//针对10M带宽的修改
            {
                ...
                //网外干扰：
                //斜率（9-49）<-0.10  and 斜率（50-89）>0.10  and聚类突变top数量值<10 and峰值指示向量不存在（20-80）中
                if (slope9to49 < -0.10  && ClusterMutationTopCount < 10 && !fun1(peakindicatorvector, 20, 50) && standarddeviation < 2.0) {
                    return 9;
                }
                //杂散干扰：
                //IF  |斜率|>K6 0.12  and平滑度>K7 0.86 and标准差>2.8 and 聚类突变top数量值<4 and最后15个prb均值-前面85个PRB均值<4 and 峰值指示向量不存在（20-50）中
                //RB0~RB9的均值高于RB45~RB54的均值4dB.
                else if ((avg0to9-avg45to49)>4 && slope > 0.12 && smoothness > 0.86 && standarddeviation > 2.8 && ClusterMutationTopCount < 4  && !fun1(peakindicatorvector, 20, 50) && ave_if_nl < -100.0) {
                    return 1;
                }
                //GPS干扰：
                //IF 49 or 50处为峰值and聚类突变top数量值<10，并且本小区相邻10公里的小区中有存在GPS故障小区。
                ...
                //IF -0.15<斜率<0.15 and平滑度>0.9 and 0-15存在峰值指示量and 相关性系数>0.60 and 均值(10-89prb)>-110 and 聚类突变top数量值<10  and  |斜率（9-49）|<0.1 and|斜率（50-89）|<0.1
                //RB0~RB9的均值高于RB45~RB54的均值4dB.
                else if ((avg0to9-avg45to49)>4 && (xgxs == 1111.0 || xgxs > 0.6) && slope > -0.15 && slope < 0.15 && smoothness > 0.9 && fun1(peakindicatorvector, 0, 15) && ClusterMutationTopCount < 10 && Math.abs(slope9to49) < 0.1 && Math.abs(slope50to90) < 0.1) {
                    return 6;
                }
                //宽频干扰：
                //IF |斜率|<0.1 and and |斜率（9-49）|<0.1 and|斜率（50-89）|<0.1 and平滑度>0.95  and 标准差<2.5 and最后15个prb均值-前面85个PRB均值<5 and相关性系数<0.6 and 均值(10-89prb)>-110 and 峰值指示向量数（参数为2）<3 and聚类突变top数量值<10，则判断为网外干扰-宽频干扰 。
                //需求中均值(10-50prb)>-110 条件即为   avg10to89 > -110.0   因为IF_AvgValue_UDF方法算有效值小于55个的时候是按有效个数求均值
                else if ( avg10to89 > -110.0 && Math.abs(slope) < 0.1 && Math.abs(slope9to49) < 0.1  && smoothness > 0.95 && standarddeviation < 2.5  && peakindicatorvector_2.split(",").length < 3 && ClusterMutationTopCount < 10 && (xgxs == 1111.0 || xgxs < 0.6) && ave_if_nl > -95.0) {
                    return 5;
                }
                //互调干扰：
                //IF峰值数量(参数为2的)大于等于3个 and 8、9、10、18、19、20时段存在干扰 and聚类突变top数量值<15  and最后15个prb均值-前面85个PRB均值<4
                else if (peakindicatorvector_2.split(",").length >= 3 && ClusterMutationTopCount < 15  && ave_if_nl < -95.0) {
                    return 3;
                }
                //广电干扰：
                //IF  平滑度>0.80   and 20<聚类突变top数量值<70
