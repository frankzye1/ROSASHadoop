...
public class IF_GetTypeNo_UDF extends UDF {
    public int evaluate(double avg10to89 ,double slope9to49,double slope50to90,int ClusterMutationTopCount,int max_no, double slope, double smoothness, double standarddeviation, String peakindicatorvector, String peakindicatorvector_2, double top40indexaggregation, double top15andtail15, String flag, double xgxs, double Top40AndLast60Broadcast, double AvgTail15_85) {
        try {
            ...
            else if (Math.abs(slope) > 0.12 && smoothness > 0.86 && standarddeviation > 2.8 && ClusterMutationTopCount<4&&AvgTail15_85<4&&!fun1(peakindicatorvector,20, 80)) {
                return 1;
            }
            //GPS干扰：
            //IF 49 or 50处为峰值and聚类突变top数量值<10，并且本小区相邻10公里的小区中有存在GPS故障小区。
            else if ((peakindicatorvector.indexOf("49") >= 0 || peakindicatorvector.indexOf("50") >= 0) && ClusterMutationTopCount<10 && flag == "1") {
                return 2;
            }
            //同频干扰：
            //IF  {|斜率|<0.1 and平滑度>0.80 and 47 or 48 or 49 or 50 or 51处为峰值 and 峰值向量数<10聚类突变top数量值<10)} or {峰值指示向量只存在（47、48、49、50、51）}中 。
                    ...
                    (Math.abs(slope) < 0.1 && smoothness > 0.80&& peakindicatorvector.split(",").length<10 && ClusterMutationTopCount<10
                            && (peakindicatorvector.indexOf("49") >= 0 || peakindicatorvector.indexOf("50") >= 0 || peakindicatorvector.indexOf("47") >= 0 || peakindicatorvector.indexOf("51") >= 0 || peakindicatorvector.indexOf("48") >= 0))
                    || Arrays.asList("47,48,49,50,51".split(",")).containsAll(Arrays.asList(peakindicatorvector.split(",")))
                    ) {
                return 4;
            }
            //阻塞干扰：
            ...
            else if (Math.abs(slope) < 0.1 && Math.abs(slope9to49) < 0.1 && Math.abs(slope50to90) < 0.1 && smoothness > 0.95 && standarddeviation < 2.5 && AvgTail15_85 < 5.0 && avg10to89 > -110.0 && peakindicatorvector_2.split(",").length < 3 && ClusterMutationTopCount < 10 && (xgxs == 1111.0 || xgxs < 0.6)) {
                return 5;
            }
            //互调干扰：
            //IF峰值数量(参数为2的)大于等于3个 and 8、9、10、18、19、20时段存在干扰 and聚类突变top数量值<15  and最后15个prb均值-前面85个PRB均值<4
            else if (peakindicatorvector_2.split(",").length >= 3  && ClusterMutationTopCount < 15&& AvgTail15_85<4) {
                return 3;
            }
            //广电干扰：
            //IF  平滑度>0.80   and 20<聚类突变top数量值<70
            else if (smoothness > 0.8    && (ClusterMutationTopCount > 20 && ClusterMutationTopCount < 70)) {
                return 7;
            }
            //PHS干扰：
